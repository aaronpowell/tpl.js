(function (global) {
    ...
    var tpl = function (array, fn, processes) {
        ...
        var workers = x.map(function (item) {
          return {
            blob: new Blob([txt]),
            val: item,
            defer: Q.defer()
            ...
            obj.blob = global.URL.createObjectURL(obj.blob);
            return obj;
        }).map(function (obj) {
            var worker = new Worker(obj.blob);
            worker.onmessage = function (e) {
                obj.defer.resolve(e.data);
            };
            setTimeout(function () {
                worker.postMessage(obj.val);
            }, 0);

            return {
                worker: worker,
                promise: obj.defer.promise
            };
        });
